...
import {
  FIN_SUPPORT_REP_DB_HELP,
  FIN_SUPPORT_REP_DB_HELP_SUB,
  FIN_SUPPORT_REP_DB_HELP_TITLE,
} from "@/lib/analysis/financial-support-benefit-rate-rep-db-help";
import {
  FDB_PAGE_SHELL,
          ...
          <tbody>
            {rows.map((row, i) => (
              ...
              >
                ...
                <td className={metricCell}>
                  {fmtMillion1(wonToMillion1(row.nationalScholarship))}
                </td>
                <td className={metricCell}>
                  {fmtMillion1(wonToMillion1(row.centralSubtotal))}
                </td>
                <td className={metricCell}>
                  {fmtMillion1(wonToMillion1(row.localGovernment))}
                </td>
                <td className={metricCell}>
                  {fmtMillion1(wonToMillion1(row.totalSupport))}
                </td>
                <td className={metricCell}>
                  {fmtMillion1(cheonToMillion1(row.tuitionRevenue))}
                </td>
                <td className={`${metricCell} ${FDB_TABLE_COLOR.ratePrimary}`}>
                  {fmtRate(row.benefitRate)}
                </td>
              </tr>
            ))}
          </tbody>
...
export function FinancialSupportBenefitRateRepMockPage({
  data,
  variant = "mock",
}: {
  data: FinSupportRepMockData;
  ...
  return (
    <div className={FDB_PAGE_SHELL}>
      ...
      {variant === "mock" ? (
        <section className="rounded-xl border border-dashed border-accent/40 bg-accent/5 px-4 py-3">
          <p className={`${FDB_TYPO.panelMeta} text-foreground`}>
            ...
            대표학교코드로 재정알리미 재정지원·교비자금 원본을 합산한
            목업입니다.
          </p>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <RepDbDownButton
          variant="glass"
          href="/api/ingest/finance-analysis/financial-support-benefit-rate/rep/export"
          download="financial_support_benefit_rate_rep_db.xlsx"
        />
      </div>
      ...
      {data.section === "charts" ? (
        ...
        <>
          ...
          {dbHelpOpen ? (
            <HelpGuidePanel
              sections={FIN_SUPPORT_REP_DB_HELP}
              onClose={() => setDbHelpOpen(false)}
              eyebrow={FIN_SUPPORT_REP_DB_HELP_TITLE}
              title="대표학교 합산 규칙"
              description={FIN_SUPPORT_REP_DB_HELP_SUB}
            />
          ) : null}
