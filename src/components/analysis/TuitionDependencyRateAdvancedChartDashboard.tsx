...
import {
  ...
  fmtYoy,
  type BoxPlotStats,
  type TuitionDependencyAdvancedFilters,
  type RegionAggregate,
  type TuitionDependencyRateAdvancedRow,
} from "@/lib/analysis/tuition-dependency-rate-advanced-analytics";
import { CHART_THEME } from "@/lib/theme/teal-glow";
import {
  TUITION_DEPENDENCY_ADVANCED_CHART_HELP,
  TUITION_DEPENDENCY_ADVANCED_HELP_SECTIONS,
  TUITION_DEPENDENCY_ADVANCED_KPI_HELP,
  TUITION_DEPENDENCY_ADVANCED_TAB_HELP,
  type HelpSection,
} from "@/lib/analysis/tuition-dependency-rate-advanced-help";
import {
...
import { TuitionDependencyRateDensityDistributionChart } from "@/components/analysis/TuitionDependencyRateDensityDistributionChart";
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className={CHART_TYPO.legend}>
        그룹별 등록금의존율(%) 분포 — 상자=중간 10%, 가로선=중앙값, 양끝 수염=일반 범위
      </p>
    </div>
  );
}
  ...
  return (
    <div className="overflow-x-auto">
      <table className={`w-full table-fixed border-collapse ${CHART_TYPO.tableBody}`}>
        ...
        <thead>
          <tr className="border-b border-border bg-surface-2 text-center">
            ...
            <th className="px-3 py-2 font-medium">평균 등록금의존율</th>
      ...
      <p className={`mt-2 ${CHART_TYPO.legend}`}>
        평균 등록금의존율은 가중 평균, 평균값은 학교별 산술평균입니다. 행 클릭 시 하단 위험군 대학 목록이 해당 지역으로 필터링됩니다.
      </p>
    </div>
  );
}
...
function RiskSchoolList({
  ...
  const riskRows = rows
    ...
    .sort((a, b) => b.tuitionDependencyRate - a.tuitionDependencyRate);
  ...
  return (
    <div>
        ...
        <table className={`w-full min-w-[960px] border-collapse ${CHART_TYPO.tableBody}`}>
          <thead className="sticky top-0 z-10 bg-surface-2">
            <tr className="border-b border-border">
              ...
              <th className="px-2 py-2 text-center">운영수입합계(백만원)</th>
              <th className="px-2 py-2 text-center">등록금의존율</th>
            ...
            {riskRows.map((r) => {
              ...
              return (
                <tr key={r.schoolCodeStd} className="border-b border-border/30">
                  ...
                  >
                    {fmtPct(r.tuitionDependencyRate)}
                  </td>
      ...
      <p className={`mt-2 ${CHART_TYPO.legend}`}>
        ...
        {" · "}
        금액 단위: 백만원 (천원 ÷ 1,000) · 등록금의존율 = 등록금수입 ÷ 운영수입합계 × 100
      </p>
    </div>
  );
}
...
type TuitionDependencyRateAdvancedChartDashboardProps = {
  rows: TuitionDependencyRateAdvancedRow[];
  years: number[];
  hasData?: boolean;
...
export function TuitionDependencyRateAdvancedChartDashboard({
  ...
  const zoneComparePanel = (
    <PanelWithHelp
      title="5극 3특 권역 비교"
      help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.zoneCompare}
    >
      <GeoCompareComposedChart
        data={zoneBarData}
        avgRateName="평균 등록금의존율"
        xAxisAngle={-20}
        xAxisHeight={48}
        barCategoryGap="18%"
        maxBarSize={32}
  ...
  const scaleComparePanel = (
    <PanelWithHelp
      title="학생 규모 비교"
      help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.scaleCompare}
    >
      <GeoCompareComposedChart
        data={scaleBarData}
        avgRateName="평균 등록금의존율"
  ...
  const sidoRankPanel = (
    <PanelWithHelp
      title="17개 시·도 순위"
      help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.sidoRank}
    >
      <GeoCompareComposedChart
        data={sidoBarData}
        avgRateName="평균 등록금의존율"
  ...
  const riskTierPanel = (
    <PanelWithHelp
      ...
      help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.riskTier}
    >
  ...
  const histogramPanel = (
    <PanelWithHelp
      ...
      help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.histogram}
    >
      <div className={RISK_HISTOGRAM_SPLIT.heightClass}>
        <ResponsiveContainer width="100%" height="100%">
  ...
  const densityDistributionPanel = (
    <PanelWithHelp
      title="등록금의존율 밀도 분포 (Density / Violin)"
      subtitle="대학/그룹 수 × 등록금의존율(%) — 하위 25%·중앙값·상위 25%·평균"
      help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.density}
    >
      <TuitionDependencyRateDensityDistributionChart
        rates={currentRows.map((r) => r.tuitionDependencyRate)}
  ...
  const mainTabs: { id: MainTab; label: string; help: HelpSection }[] = [
    ...
    { id: "geo", label: "지역·규모", help: TUITION_DEPENDENCY_ADVANCED_TAB_HELP.geo },
    {
      id: "distribution",
      label: "분포·위험",
      help: TUITION_DEPENDENCY_ADVANCED_TAB_HELP.distribution,
    },
    {
      id: "pipeline",
      label: "시계열",
      help: TUITION_DEPENDENCY_ADVANCED_TAB_HELP.pipeline,
    },
  ];
  ...
  if (!hasData || chartYears.length === 0) {
    return (
      <section className="rounded-xl border border-border bg-surface p-8 text-center">
        <p className={CHART_TYPO.bodyText}>
          등록금의존율 데이터가 없습니다. 대학별DB 탭에서 엑셀 데이터를 업로드하세요.
        </p>
      </section>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {helpOpen ? (
        <HelpGuidePanel
          sections={TUITION_DEPENDENCY_ADVANCED_HELP_SECTIONS}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          sub="Σ운영수입합계 ÷ Σ등록금수입 · 높을수록 좋음"
          accent="mint"
          invertDelta
          help={TUITION_DEPENDENCY_ADVANCED_KPI_HELP.avgRate}
        />
        <KpiCard
          label="중앙값 & IQR"
          ...
          accent="blue"
          help={TUITION_DEPENDENCY_ADVANCED_KPI_HELP.medianIqr}
        />
        <KpiCard
          label="위험군 대학 수"
          value={`${kpis.riskAbove50.toLocaleString("ko-KR")}개교`}
          sub={formatCohortRiskKpiSub(cohortCutoffs)}
          accent="rose"
          help={TUITION_DEPENDENCY_ADVANCED_KPI_HELP.riskCount}
        />
        <KpiCard
          label="분석 대상"
          ...
          sub={`${year}년 · 선택 필터 적용`}
          accent="amber"
          help={TUITION_DEPENDENCY_ADVANCED_KPI_HELP.schoolCount}
        />
      </div>
      ...
      {mainTab === "risk" ? (
        <>
          <PanelWithHelp
            ...
            help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.sidoTable}
          >
            <RegionalTable
              rows={sidoAgg}
              total={totalAgg}
          ...
          <PanelWithHelp
            title="위험군 대학 목록"
            help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.schoolPreview}
          >
          ...
          <>
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)] lg:items-start">
              <PanelWithHelp
                ...
                help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.boxPlot}
              >
      ...
      {mainTab === "pipeline" ? (
        <div className="grid gap-4 lg:grid-cols-2 lg:items-start">
          <PanelWithHelp
            title="5개년 권역별 추이"
            subtitle="5극 3특 권역 평균 등록금의존율 · 변동성 비교"
            help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.trend}
          >
          ...
          <PanelWithHelp
            title="5개년 규모별 추이"
            subtitle="대규모·중규모·소규모 평균 등록금의존율 · 변동성 비교"
            help={TUITION_DEPENDENCY_ADVANCED_CHART_HELP.funnel}
          >
      ...
      <section className={`rounded-lg border border-accent-orange/30 bg-accent-orange/5 px-4 py-3 ${CHART_TYPO.bodyText}`}>
        ...
        <ul className="mt-2 list-inside list-disc space-y-1">
          <li>
            Gemini 기획안을 <strong className="text-foreground">등록금의존율</strong> 지표에
            맞게 재구성한 고도화 통계분석 목업입니다.
          </li>
          <li>
            충원율·정원 구분 등 입시 지표는 제외하고, 등록금의존율·권역/시도
            다차원 분석으로 구성했습니다.
          </li>
