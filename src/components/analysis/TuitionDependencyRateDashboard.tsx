...
import { TuitionDependencyRateDataTable } from "@/components/analysis/TuitionDependencyRateDataTable";
import { buildTuitionDependencyRateHref } from "@/lib/analysis/tuition-dependency-rate-navigation";
import { serializeMultiFilterParam } from "@/lib/analysis/table-filter-utils";
import type {
  TuitionDependencyRateDashboardData,
  TuitionDependencyRateSection,
} from "@/lib/data/tuition-dependency-rate";
import {
  TUITION_DEPENDENCY_RATE_HELP_LINES,
  TUITION_DEPENDENCY_RATE_TEMPLATE_HEADER,
  TUITION_DEPENDENCY_RATE_TEMPLATE_SAMPLES,
} from "@/lib/ingest/tuition-dependency-rate-config";
...
function TemplatePreviewTable() {
  return (
    <div className="mt-4 w-full basis-full overflow-x-auto rounded-lg border border-border/60">
      <table className={`w-full min-w-[1100px] border-collapse text-left ${FDB_TYPO.tableBody}`}>
        <thead className="border-b border-border bg-surface-2">
          <tr>
            {TUITION_DEPENDENCY_RATE_TEMPLATE_HEADER.map((h) => (
              <th
                key={h}
        ...
        <tbody>
          {TUITION_DEPENDENCY_RATE_TEMPLATE_SAMPLES.map((row, i) => (
            <tr key={i} className="border-b border-border/40 text-muted">
              ...
              <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
                {fmtAmount(row.교비_운영수입)}
              </td>
              <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
                {fmtAmount(row.산단_운영수입)}
              </td>
              <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
                {fmtAmount(row.운영수입합계)}
              </td>
              <td className="px-2 py-1.5 text-right font-mono font-semibold text-accent-orange">
                {fmtPercent(row.등록금의존율)}
              </td>
            </tr>
          ))}
        </tbody>
    ...
    startTransition(async () => {
      try {
        const res = await fetch(
          "/api/ingest/finance-analysis/tuition-dependency-rate/upload",
          { method: "POST", body: fd },
        );
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <h4 className={`mt-1 ${FDB_TYPO.panelTitle}`}>등록금의존율</h4>
            <p className={`mt-2 max-w-xl ${FDB_TYPO.bodyText}`}>
              등록금의존율 엑셀을 업로드하면{" "}
              <code className="text-accent">
                data/csv/finance_analysis_tuition_dependency_rate.csv
              </code>
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
            <UploadPanelTemplateLink href="/api/ingest/finance-analysis/tuition-dependency-rate/template" download="tuition_dependency_rate_upload_template.xlsx" />
            <input
              ref={inputRef}
              type="file"
              accept=".xlsx,.xls,.csv"
        ...
        <FinanceAnalysisDbExportButtons
          exportBasePath="/api/ingest/finance-analysis/tuition-dependency-rate/export"
          campusRowCount={rowCount}
        />
        {helpOpen ? (
          <div className={`rounded-lg border border-border/60 bg-surface-2/50 p-4 ${FDB_TYPO.bodyText}`}>
            {TUITION_DEPENDENCY_RATE_HELP_LINES.map((line, i) => (
            ...
            <p className="mt-2">
              ...
              기준연도·학교코드_표준·학교명·학교구분명·학교종류명·지역명·설립구분명·등록금수입·교비_운영수입·산단_운영수입·운영수입합계·등록금의존율
...
export function TuitionDependencyRateDashboard({
  data,
}: {
  data: TuitionDependencyRateDashboardData;
}) {
  const router = useRouter();
  ...
  function navigate(next: {
    ...
    router.push(
      buildTuitionDependencyRateHref({
          ...
          <div className="flex flex-col gap-4">
            ...
            <section className="rounded-xl border border-border bg-surface p-5">
              {!data.hasData || displayYear == null ? (
                ...
                <TuitionDependencyRateDataTable rows={filteredRows} />
              )}
            </section>
          </div>
        )}
