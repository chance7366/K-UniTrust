...
import { fmtPct } from "@/lib/analysis/fund-secure-rate-advanced-analytics";
import {
  buildFundSecureRateDensity,
  type FundSecureRateDensityStats,
  type RateDensityScale,
} from "@/lib/analysis/fund-secure-rate-density-chart";
...
type Props = {
  rates: number[];
  densityScale?: Partial<RateDensityScale>;
  rateDigits?: number;
  xAxisLabel?: string;
  /** 전국 가중평균(KPI) — 지정 시 단순평균 대신 표시 */
  weightedMean?: number | null;
  /** 동종 상위 15%/7% 선 (지표값). 있으면 150%+ 카드 대신 표시 */
  cohortTopCutoffs?: { top15: string; top7: string } | null;
};

function fmtRatePct(v: number, digits: number): string {
...
function scaleX(rate: number, stats: FundSecureRateDensityStats) {
  const pad = 8;
  const span = stats.displayXMax - stats.displayXMin || 1;
  return pad + ((rate - stats.displayXMin) / span) * (100 - pad * 2);
}

function scaleY(count: number, stats: FundSecureRateDensityStats) {
  const base = 82;
  const top = 12;
  return base - (count / stats.yMax) * (base - top);
}

function buildAreaPath(stats: FundSecureRateDensityStats): string {
  const baseline = 82;
...
export function FundSecureRateDensityDistributionChart({
  rates,
  densityScale,
  rateDigits = 1,
  xAxisLabel = "자금확보율 (%)",
  ...
  const stats = useMemo(() => {
    const built = buildFundSecureRateDensity(rates, densityScale);
    if (!built) return null;
    if (weightedMean != null && Number.isFinite(weightedMean)) {
      return { ...built, mean: weightedMean };
    }
    return built;
  }, [rates, densityScale, weightedMean]);

  if (!stats) {
    return <p className="text-sm text-muted">표시할 분포 데이터가 없습니다.</p>;
  }

  const compactScale = stats.displayXMax <= 20;
  const formatPct = (v: number) =>
    compactScale ? fmtRatePct(v, rateDigits) : fmtPct(v);
  ...
  const outlierRegionStart = compactScale
    ? scaleX(Math.max(stats.displayXMax * 0.75, 3), stats)
    : scaleX(150, stats);

  const tailStatLabel = compactScale ? "≥3% (여유)" : "150%+ (초고성과)";
  const tailStatCount = compactScale
    ? rates.filter((r) => r >= 3).length
    : stats.outlierCount;
    ...
    <div className="flex flex-col gap-4">
      <div className={`grid gap-3 ${cohortTopCutoffs ? "sm:grid-cols-2 xl:grid-cols-5" : "sm:grid-cols-4"}`}>
        ...
        {cohortTopCutoffs ? (
          ...
          <div className="rounded-lg border border-border/60 bg-surface-2/40 px-3 py-2">
            <p className={FDB_TYPO.legend}>{tailStatLabel}</p>
            <p className="font-mono text-lg font-semibold text-rose-600">
              {tailStatCount.toLocaleString("ko-KR")}개교
            </p>
          </div>
        )}
      </div>
      ...
      <div className="relative h-[360px] w-full rounded-lg border border-border/60 bg-surface-2/20 p-2">
        ...
        >
          ...
          {!compactScale ? (
            <rect
              x={outlierRegionStart}
              y="12"
              width={96 - outlierRegionStart}
              height="70"
              fill={`${CHART.rose}11`}
            />
          ) : null}
      ...
      <p className={FDB_TYPO.legend}>
        Y=대학/그룹 수, X={xAxisLabel} — 표시 구간 {stats.displayXMin}%~
        {stats.displayXMax}%
        {stats.rawMax > stats.displayXMax
          ? ` (일부 극단값 ${formatPct(stats.rawMax)}는 우측 꼬리에 합산)`
          : ""}
        .
      </p>
    </div>
