...
import { FundSecureRateDataTable } from "@/components/analysis/FundSecureRateDataTable";
import { SchoolNameSearchInput } from "@/components/analysis/SchoolNameSearchInput";
import { buildFundSecureRateHref } from "@/lib/analysis/fund-secure-rate-navigation";
import { serializeMultiFilterParam } from "@/lib/analysis/table-filter-utils";
import type {
  FundSecureRateDashboardData,
  FundSecureRateSection,
} from "@/lib/data/fund-secure-rate";
import {
  FUND_SECURE_RATE_HELP_DESCRIPTION,
    ...
    <div className="mt-4 w-full basis-full overflow-x-auto rounded-lg border border-border/60">
      <table className={`w-full min-w-[1200px] border-collapse text-left ${FDB_TYPO.tableBody}`}>
        <thead className="border-b border-border bg-surface-2">
          <tr>
            {FUND_SECURE_RATE_TEMPLATE_HEADER.map((h) => (
              <th
                key={h}
        ...
        <tbody>
          {FUND_SECURE_RATE_TEMPLATE_SAMPLES.map((row, i) => (
            <tr key={i} className="border-b border-border/40 text-muted">
              ...
              <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
                {fmtAmount(row.교비_이월자금)}
              </td>
              <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
                {fmtAmount(row.교비_기금)}
              </td>
              <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
                {fmtAmount(row.산단_이월자금)}
              </td>
              <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
                {fmtAmount(row.산단_기금)}
              </td>
    ...
    startTransition(async () => {
      try {
        const res = await fetch(
          "/api/ingest/finance-analysis/fund-secure-rate/upload",
          { method: "POST", body: fd },
        );
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <h4 className={`mt-1 ${FDB_TYPO.panelTitle}`}>자금확보율</h4>
            <p className={`mt-2 max-w-xl ${FDB_TYPO.bodyText}`}>
              자금확보율 엑셀을 업로드하면{" "}
              <code className="text-accent">
                data/csv/finance_analysis_fund_secure_rate.csv
              </code>
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
            <UploadPanelTemplateLink href="/api/ingest/finance-analysis/fund-secure-rate/template" download="fund_secure_rate_upload_template.xlsx" />
            <input
              ref={inputRef}
              type="file"
              accept=".xlsx,.xls,.csv"
        ...
        <FinanceAnalysisDbExportButtons
          exportBasePath="/api/ingest/finance-analysis/fund-secure-rate/export"
          campusRowCount={rowCount}
        />
        {helpOpen ? (
          <div className={`rounded-lg border border-border/60 bg-surface-2/50 p-4 ${FDB_TYPO.bodyText}`}>
            <p>{FUND_SECURE_RATE_HELP_DESCRIPTION}</p>
            <p className="mt-2">
              <span className="font-medium text-foreground">업로드 양식</span>은
              기준연도·학교코드_표준·학교명·학교구분명·학교종류명·지역명·설립구분명·교비_이월자금·교비_기금·산단_이월자금·산단_기금·자금합계·등록금수입·자금확보율
...
export function FundSecureRateDashboard({
  data,
}: {
  data: FundSecureRateDashboardData;
}) {
  const router = useRouter();
    ...
    <div className="flex w-full flex-col gap-4 pb-10">
      ...
      {section === "charts" ? (
        ...
        {!data.hasData || displayYear == null ? (
          ...
          <FundSecureRateDataTable rows={filteredRows} />
        )}
          </section>
        </div>
        )}
