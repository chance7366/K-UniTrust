...
import {
  buildFinancialSupportBenefitRateDensity,
  type FinancialSupportBenefitRateDensityStats,
} from "@/lib/analysis/financial-support-benefit-rate-density-chart";
import { fmtPct } from "@/lib/analysis/financial-support-benefit-rate-advanced-analytics";
...
function scaleX(rate: number, stats: FinancialSupportBenefitRateDensityStats) {
  const pad = 8;
  const span = stats.displayXMax - stats.displayXMin || 1;
  return pad + ((rate - stats.displayXMin) / span) * (100 - pad * 2);
}

function scaleY(count: number, stats: FinancialSupportBenefitRateDensityStats) {
  const base = 82;
  const top = 12;
  return base - (count / stats.yMax) * (base - top);
}

function buildAreaPath(stats: FinancialSupportBenefitRateDensityStats): string {
  const baseline = 82;
...
export function FinancialSupportBenefitRateDensityDistributionChart({
  rates,
  weightedMean,
  cohortTopCutoffs,
}: Props) {
  const stats = useMemo(() => {
    const built = buildFinancialSupportBenefitRateDensity(rates);
  ...
  const outlierRegionStart = scaleX(200, stats);
    ...
    <div className="flex flex-col gap-4">
      <div className={`grid gap-3 ${cohortTopCutoffs ? "sm:grid-cols-2 xl:grid-cols-5" : "sm:grid-cols-4"}`}>
        ...
        {cohortTopCutoffs ? (
          ...
          <div className="rounded-lg border border-border/60 bg-surface-2/40 px-3 py-2">
            <p className={FDB_TYPO.legend}>100%+ (등록금 이상 지원)</p>
            <p className="font-mono text-lg font-semibold text-emerald-600">
              {stats.highPerformerCount.toLocaleString("ko-KR")}개교
            </p>
          </div>
        )}
      </div>
      ...
      <div className="relative h-[360px] w-full rounded-lg border border-border/60 bg-surface-2/20 p-2">
        ...
        >
          ...
          {/* X grid (labels rendered in HTML overlay for 11px) */}
          {xTicks.map((val) => {
        ...
        {/* HTML annotations + axis labels (11px, non-distorted) */}
      ...
      <p className={FDB_TYPO.legend}>
        Y=대학/그룹 수, X=재정지원수혜율(%) — 표시 구간 {stats.displayXMin}%~
        {stats.displayXMax}%
        {stats.rawMax > stats.displayXMax
          ? ` (일부 극단값 ${stats.rawMax}%는 우측 꼬리에 합산)`
          : ""}
        . &lt;70% {stats.below70Count.toLocaleString("ko-KR")}개교.
      </p>
    </div>
  );
}
