...
import { IncomePropertySecureRateDataTable } from "@/components/analysis/IncomePropertySecureRateDataTable";
import { SchoolNameSearchInput } from "@/components/analysis/SchoolNameSearchInput";
import { priorTuitionYear } from "@/lib/analysis/income-property-secure-rate-analytics";
import { buildIncomePropertySecureRateHref } from "@/lib/analysis/income-property-secure-rate-navigation";
import { serializeMultiFilterParam } from "@/lib/analysis/table-filter-utils";
import type {
  IncomePropertySecureRateDashboardData,
  IncomePropertySecureRateSection,
} from "@/lib/data/income-property-secure-rate";
import {
  INCOME_PROPERTY_HELP_LINES,
...
import { fmtMillionWon } from "@/lib/analysis/income-property-secure-rate-analytics";
...
function TemplatePreviewTable() {
  const sample = INCOME_PROPERTY_TEMPLATE_SAMPLES[0];
  return (
    <div className="mt-4 w-full basis-full overflow-x-auto rounded-lg border border-border/60">
      <table className={`w-full min-w-[1400px] border-collapse text-left ${FDB_TYPO.tableBody}`}>
        <thead className="border-b border-border bg-surface-2">
          <tr>
            {INCOME_PROPERTY_SECURE_RATE_TEMPLATE_HEADER.map((h) => (
              <th
                key={h}
        ...
        <tbody>
          <tr className="border-b border-border/40 text-muted">
            ...
            <td className="border-r border-border/40 px-2 py-1.5">{sample.학교명}</td>
            <td className="border-r border-border/40 px-2 py-1.5">{sample.법인명}</td>
            <td className="border-r border-border/40 px-2 py-1.5">{sample.학교구분명}</td>
            <td className="border-r border-border/40 px-2 py-1.5">{sample.지역명}</td>
            <td className="border-r border-border/40 px-2 py-1.5">{sample.설립구분명}</td>
            <td className="border-r border-border/40 px-2 py-1.5">{sample.학교상태명}</td>
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
              {sample.토지_평가액.toLocaleString("ko-KR")}
            </td>
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
              {sample.토지_순수입액.toLocaleString("ko-KR")}
            </td>
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
              {sample.건물_평가액.toLocaleString("ko-KR")}
            </td>
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
              {sample.건물_순수입액.toLocaleString("ko-KR")}
            </td>
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
              {sample["유가증권-평가액"]}
            </td>
            ...
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
              {sample.예금_평가액.toLocaleString("ko-KR")}
            </td>
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
              {sample.예금_순수입액.toLocaleString("ko-KR")}
            </td>
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
              {sample.기타재산_평가액}
            </td>
            ...
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono text-yellow-400">
              {sample.담보차감액.toLocaleString("ko-KR")}
            </td>
            <td className="border-r border-border/40 px-2 py-1.5 text-right font-mono">
              {sample["평가액 합계"].toLocaleString("ko-KR")}
            </td>
            <td className="px-2 py-1.5 text-right font-mono">
              {sample["순수입액 합계"].toLocaleString("ko-KR")}
            </td>
          </tr>
        </tbody>
      </table>
      <p className={`border-t border-border/40 px-3 py-2 ${FDB_TYPO.legend}`}>
        양식 헤더·샘플 행 — 업로드 시 1행 헤더 구조를 그대로 사용하세요. I~U열
        금액 단위는 천원입니다. 화면 표시는 백만원(반올림)입니다.
      </p>
      <p className={`border-t border-border/40 px-3 py-2 ${FDB_TYPO.legend}`}>
        재산확보율 계산: {sample.조사년도}년 평가액 합계({fmtMillionWon(sample["평가액 합계"])}백만) ÷{" "}
        {priorTuitionYear(sample.조사년도)}년 등록금수입(자금확보율 DB)
      </p>
    </div>
  );
}
    ...
    startTransition(async () => {
      try {
        const res = await fetch(
          "/api/ingest/finance-analysis/income-property-secure-rate/upload",
          { method: "POST", body: fd },
        );
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
            <UploadPanelTemplateLink href="/api/ingest/finance-analysis/income-property-secure-rate/template" download="income_property_secure_rate_upload_template.xlsx" />
            <input
              ref={inputRef}
              type="file"
              accept=".xlsx,.xls,.csv"
        ...
        <FinanceAnalysisDbExportButtons
          exportBasePath="/api/ingest/finance-analysis/income-property-secure-rate/export"
          campusRowCount={rowCount}
        />
        {helpOpen ? (
          <div className={`rounded-lg border border-border/60 bg-surface-2/50 p-4 ${FDB_TYPO.bodyText}`}>
            {INCOME_PROPERTY_HELP_LINES.map((line, i) => (
            ...
            <p className="mt-2">
              ...
              은 백만원 환산 금액과 등록금수입·재산확보율·수익율 계산값을 포함한
              엑셀 파일을 받을 수 있습니다.
            </p>
          </div>
        ) : null}
...
export function IncomePropertySecureRateDashboard({
  data,
}: {
  data: IncomePropertySecureRateDashboardData;
}) {
  const router = useRouter();
  ...
  function navigate(next: {
    ...
    router.push(
      buildIncomePropertySecureRateHref({
    ...
    <div className="flex w-full flex-col gap-4 pb-10">
      ...
      <section className="rounded-xl border border-border bg-surface p-5">
        {!data.hasData || displayYear == null ? (
          ...
          <IncomePropertySecureRateDataTable
            rows={filteredRows}
