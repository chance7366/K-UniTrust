...
export type FreshmanEnrollmentViewMode = "campus" | "consolidated";

export type FreshmanEnrollmentSection = "charts" | "data";

export type FreshmanEnrollmentQuery = {
  year?: number | null;
  estb?: string;
...
export type FreshmanEnrollmentFilterOptions = {
  estbs: string[];
...
export type FreshmanEnrollmentYearStatus = {
  year: number;
...
export type FreshmanEnrollmentDashboardData = {
  years: number[];
  displayYear: number | null;
  rows: FreshmanEnrollmentRow[];
  consolidatedRows: FreshmanEnrollmentConsolidatedRow[];
  allCampusRows: FreshmanEnrollmentRow[];
  allConsolidatedRows: FreshmanEnrollmentConsolidatedRow[];
  yearStatuses: FreshmanEnrollmentYearStatus[];
  viewMode: FreshmanEnrollmentViewMode;
  section: FreshmanEnrollmentSection;
  hasAnyConsolidatedData: boolean;
  filterOptions: FreshmanEnrollmentFilterOptions;
  yearRowCount: number;
  filters: {
    estb: string;
    schoolDivision: string;
...
function parseCampusRow(r: Record<string, string>): FreshmanEnrollmentRow | null {
  ...
  return {
    ...
    schoolName,
    admissionQuota: num(r.admission_quota) ?? 0,
    recruit: {
      total: num(r.recruit_total) ?? 0,
      within: num(r.recruit_within) ?? 0,
      outside: num(r.recruit_outside) ?? 0,
    },
    enrolled: {
    ...
    fillRate: {
      within: num(r.fill_rate_within) ?? 0,
      withinOutside: num(r.fill_rate_within_outside) ?? 0,
    },
  };
}
...
): FreshmanEnrollmentConsolidatedRow | null {
  const year = num(r.year);
...
export async function loadFreshmanEnrollmentDashboard(
  query: FreshmanEnrollmentQuery = {},
): Promise<FreshmanEnrollmentDashboardData> {
  const [campusRaw, consolidatedRaw, divisionLookup] = await Promise.all([
    readCsvFile("financeAnalysisFreshmanEnrollment").catch(() => []),
  ...
  const viewMode: FreshmanEnrollmentViewMode =
    query.view === "consolidated" ? "consolidated" : "campus";

  const section: FreshmanEnrollmentSection =
    query.section === "charts" ? "charts" : "data";

  const hasAnyConsolidatedData = yearStatuses.some((s) => s.hasConsolidatedData);

  const allCampusRows: FreshmanEnrollmentRow[] = [];
  const allConsolidatedRows: FreshmanEnrollmentConsolidatedRow[] = [];
  if (section === "charts") {
    for (const r of campusEnriched) {
  ...
  let yearRowCount = 0;
  const rows: FreshmanEnrollmentRow[] = [];
  const consolidatedRows: FreshmanEnrollmentConsolidatedRow[] = [];
...
export function parseFreshmanEnrollmentQuery(searchParams: {
  year?: string;
  schoolKind?: string;
  schoolDivision?: string;
  estb?: string;
