...
export async function loadFinSupportRepMockDashboard(
  ...
  const [targetRaw, supportRaw, fundRaw, supportHeaders, fundHeaders] =
    await Promise.all([
  ...
  const support = supportRaw
    .map((row) => parseAlimiFinancialSupportRow(row, supportHeaders))
    .filter((row): row is NonNullable<typeof row> => row != null);
    ...
    chartRows: years.flatMap((year) => {
      ...
      const yearRoster =
        yearRosterYear != null
          ...
          : [];
      const build = (c: FinSupportRepCohort) =>
        buildFinSupportRepRows({
          cohort: c,
          displayYear: year,
          roster: yearRoster,
