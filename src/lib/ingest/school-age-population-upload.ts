...
function validateHeaders(aoa: unknown[][]) {
  ...
  const expected = [...SCHOOL_AGE_UPLOAD_HEADERS];
  const ok =
...
function parseRows(
  ...
  for (let i = 1; i < aoa.length; i++) {
    ...
    const weight = num(row[3]);
    if (!year || !fullName) continue;

    const classified = classifySchoolAgeSido(fullName);
    if (!classified) continue;

    const mapped: Record<string, string> = {
      year: String(year),
      region_code: code,
      region: classified.short,
      region_full: classified.full,
      admission_weight: weight == null ? "" : String(weight),
      uploaded_at: uploadedAt,
    };

    let hasAge = false;
    SCHOOL_AGE_AGES.forEach((age, idx) => {
      const v = num(row[4 + idx]);
...
export type SchoolAgePopulationUploadResult = {
  rowCount: number;
  years: number[];
...
export async function ingestSchoolAgePopulationUpload(
  buffer: Buffer,
  fileName: string,
): Promise<SchoolAgePopulationUploadResult> {
  ...
  const existing = await readCsvFile("financeAnalysisSchoolAgePopulation").catch(
    () => [],
  );
  const existingNew = existing.filter(
    (r) => r.admission_weight != null && r.age_18 != null && r.age_18 !== "",
  );
  ...
  const bronzePath = await writeBronzeSnapshot({
    domainId: "univ-map",
    submenuId: "school-age-population",
    rows: parsed,
  });
...
export function buildSchoolAgePopulationTemplateBuffer(): Buffer {
