...
import {
  TUITION_DEPENDENCY_RATE_CSV_COLUMNS,
...
function validateHeaders(row0: unknown[]) {
  ...
  const expected = [...TUITION_DEPENDENCY_RATE_TEMPLATE_HEADER];
...
function parseRows(
  ...
  for (let i = 1; i < aoa.length; i++) {
    ...
    parsed.push({
      ...
      region: s(row[5]),
      estb: s(row[6]),
      tuition_revenue: String(num(row[7]) ?? ""),
      school_operating_revenue: String(num(row[8]) ?? ""),
      industry_operating_revenue: String(num(row[9]) ?? ""),
      total_operating_revenue: String(num(row[10]) ?? ""),
      tuition_dependency_rate: String(num(row[11]) ?? ""),
...
export type TuitionDependencyRateUploadResult = {
  rowCount: number;
  years: number[];
...
export async function ingestTuitionDependencyRateUpload(
  buffer: Buffer,
  fileName: string,
  options?: { replaceAll?: boolean },
): Promise<TuitionDependencyRateUploadResult> {
  ...
  const existing = options?.replaceAll
    ? []
    : await readCsvFile("financeAnalysisTuitionDependencyRate").catch(() => []);
  ...
  const bronzePath = await writeBronzeSnapshot({
    ...
    submenuId: "tuition-dependency-rate",
    rows: parsed,
  });
...
function templateSampleToRow(
  row: (typeof TUITION_DEPENDENCY_RATE_TEMPLATE_SAMPLES)[number],
): (string | number)[] {
  return TUITION_DEPENDENCY_RATE_TEMPLATE_HEADER.map((key) => row[key as keyof typeof row]);
}

export function buildTuitionDependencyRateTemplateBuffer(): Buffer {
  const aoa: (string | number)[][] = [
    [...TUITION_DEPENDENCY_RATE_TEMPLATE_HEADER],
    ...TUITION_DEPENDENCY_RATE_TEMPLATE_SAMPLES.map(templateSampleToRow),
  ];
