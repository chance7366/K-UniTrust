...
import {
  CORP_TRANSFER_RATIO_CSV_COLUMNS,
...
function validateHeaders(row0: unknown[]) {
  ...
  const expected = [...CORP_TRANSFER_RATIO_TEMPLATE_HEADER];
...
function parseRows(
  ...
  for (let i = 1; i < aoa.length; i++) {
    ...
    parsed.push({
      ...
      region: s(row[5]),
      estb: s(row[6]),
      ordinary_expense_transfer: String(num(row[7]) ?? ""),
      legal_obligation_transfer: String(num(row[8]) ?? ""),
      asset_transfer: String(num(row[9]) ?? ""),
      total_transfer: String(num(row[10]) ?? ""),
      tuition_revenue: String(num(row[11]) ?? ""),
      transfer_ratio: String(num(row[12]) ?? ""),
...
export type CorpTransferRatioUploadResult = {
  rowCount: number;
  years: number[];
...
export async function ingestCorpTransferRatioUpload(
  buffer: Buffer,
  fileName: string,
  options?: { replaceAll?: boolean },
): Promise<CorpTransferRatioUploadResult> {
  ...
  const existing = options?.replaceAll
    ? []
    : await readCsvFile("financeAnalysisCorpTransferRatio").catch(() => []);
...
function templateSampleToRow(
  row: (typeof CORP_TRANSFER_RATIO_TEMPLATE_SAMPLES)[number],
): (string | number)[] {
  return CORP_TRANSFER_RATIO_TEMPLATE_HEADER.map((key) => row[key as keyof typeof row]);
}

export function buildCorpTransferRatioTemplateBuffer(): Buffer {
  const aoa: (string | number)[][] = [
    [...CORP_TRANSFER_RATIO_TEMPLATE_HEADER],
    ...CORP_TRANSFER_RATIO_TEMPLATE_SAMPLES.map(templateSampleToRow),
  ];
