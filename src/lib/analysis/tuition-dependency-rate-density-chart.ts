...
export type TuitionDependencyRateDensityStats = {
  points: DensityPoint[];
  q1: number;
  median: number;
  q3: number;
...
/** 등록금의존율 분포 — KDE 근사(히스토그램+스무딩) + 중앙값·평균·고의존 통계 */
export function buildTuitionDependencyRateDensity(
  rates: number[],
): TuitionDependencyRateDensityStats | null {
  ...
  const highDependencyCount = rates.filter((r) => r >= 70).length;
  const lowDependencyCount = rates.filter((r) => r < 30).length;
  const above50Count = rates.filter((r) => r >= 50).length;

  const rawMin = sorted[0] ?? 0;
  const rawMax = sorted[sorted.length - 1] ?? 100;
  const p95 = percentile(sorted, 0.95);

  const displayXMin = Math.max(0, Math.floor(rawMin / 10) * 10);
  const displayXMax = Math.min(
    100,
    Math.max(80, Math.ceil(Math.max(p95, 70) / 10) * 10),
  );
