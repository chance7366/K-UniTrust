...
export function buildUniversityReportGuidelines(
  ...
  const sections: string[] = [
    ...
    "■ 1. 보고서 목차 (모든 대학 동일)",
    "",
    "· 표지 목차: 이중 번호 금지. 각 목차 항목 우측에 본문 시작 페이지 번호(표지 제외, 1부터).",
    "· A4 1장 = 본문 페이지 1장. v2 패널·지표(h4)·소절 단위로 시스템이 자동 분할·병합한다.",
    "· 본문 1~5쪽: Executive Summary → v2 Insights → Deep-Dive+표 → Decision+SWOT → Roadmap.",
    "",
    ...
    "",
    "  [페이지 채움·레이아웃 품질 원칙]",
    ...UNIVERSITY_REPORT_PAGE_LAYOUT.qualityPrinciples.map(
      (line) => `  · ${line}`,
    ),
    "",
    "■ 5b. Gemini HTML 본문 골격 (표준 골격 — 모든 대학 동일)",
    "",
    ...
    "· 문체: 경어체(「…입니다」·「…합니다」)로 통일. 동일 문단 반복 출력 금지.",
