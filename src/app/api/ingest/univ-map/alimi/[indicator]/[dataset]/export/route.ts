...
import { buildUnivAlimiRawExportBuffer } from "@/lib/ingest/univ-alimi-raw-upload";
...
export async function GET(
  ...
  try {
    const buffer = await buildUnivAlimiRawExportBuffer(indicatorRaw, dataset);
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${indicatorRaw}_${dataset}_export.xlsx"; filename*=UTF-8''${encodeURIComponent(`${title}_${label}_원본.xlsx`)}`,
      },
    });
  } catch (err) {
    const message =
