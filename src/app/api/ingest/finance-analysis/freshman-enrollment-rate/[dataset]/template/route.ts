...
import { buildFreshmanEnrollmentAlimiTemplateBuffer } from "@/lib/ingest/freshman-enrollment-alimi-upload";
...
export async function GET(
  ...
  try {
    const buffer = await buildFreshmanEnrollmentAlimiTemplateBuffer(dataset);
    const label = FRESHMAN_ENROLLMENT_ALIMI_LABEL[dataset];
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="freshman_enrollment_${dataset}_template.xlsx"; filename*=UTF-8''${encodeURIComponent(`신입생충원_${label}_양식.xlsx`)}`,
      },
    });
  } catch (err) {
    const message =
