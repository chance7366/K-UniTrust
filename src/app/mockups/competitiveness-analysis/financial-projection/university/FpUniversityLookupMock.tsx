...
export function FpUniversityLookupMock() {
  ...
  const compareSeries = useMemo(() => {
    if (lookupTab !== "diagnosis") return [];
    return (["best", "base", "worst", "stress"] as const).map((s) => ({
      scenario: s,
  ...
  }, [lookupTab, selectedUniv, cpiPct, endYear]);
    ...
    <>
      <div className="fpm-univ-banner">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <span>
            ✦ 대학별추계 UI 목업 · 대학별경쟁력과 같은 전국·지역·좌측 목록 ·{" "}
            <Link
      ...
      <div className="flex flex-col gap-4 pb-10">
        <DashboardEmeraldHeader
          sectionLabel="재정추계분석"
          title="대학별추계"
          subtitle="좌측에서 대학을 고르면 추계결과·한계진단·대응전략을 조회합니다"
        />
